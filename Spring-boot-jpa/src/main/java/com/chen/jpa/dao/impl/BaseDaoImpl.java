...
import com.chen.jpa.dao.BaseDao;
import com.chen.jpa.entity.BaseEntity;
import com.chen.jpa.entity.BaseOrderEntity;
import com.chen.jpa.filter.Filter;

/**
 * dao-基类
 * @author chen
 * @date 2019-01-03 11:08:00
 * @param <T>
 * @param <ID>
 */
public   class BaseDaoImpl<T extends BaseEntity, ID extends Serializable> implements BaseDao<T, ID>  {
    ...
    protected List<T> findList(CriteriaBuilder criteriaBuilder, CriteriaQuery<T> criteriaQuery, Predicate restrictions,
        ...
        // 设置排序条件
        if (sequencers != null) {
            List<Order> orders = addOrders(criteriaBuilder, criteriaQuery, new ArrayList<Order>(), root, sequencers);
    ...
    protected Page<T> findPage(CriteriaBuilder criteriaBuilder, CriteriaQuery<T> criteriaQuery, Predicate restrictions,
        ...
        if (StringUtils.isNotBlank(pageable.getSortProperty()) && pageable.getSortDirection() != null) {
            switch (pageable.getSortDirection()) {
                case /** 递减 */
                asc: {
                    orders.add(criteriaBuilder.asc(root.get(pageable.getSortProperty())));
                    break;
                }
                case /** 递减 */
                desc: {
                    orders.add(criteriaBuilder.desc(root.get(pageable.getSortProperty())));
                    break;
                }
                default: {
                    break;
        ...
        if (pageable.getSequencers() != null) {
            orders = addOrders(criteriaBuilder, criteriaQuery, orders, root, pageable.getSequencers());
        }
        // 设置默认排序条件
        if (orders.isEmpty()) {
            if (BaseOrderEntity.class.isAssignableFrom(entityClass)) {
                orders.add(criteriaBuilder.asc(root.get(BaseOrderEntity.ORDER_PROP)));
            } else {
                orders.add(criteriaBuilder.desc(root.get(BaseEntity.CREATE_DATE_PROP)));
            }
        }
        criteriaQuery.orderBy(orders);
     ...
     * @return 限制条件
     */
    protected Predicate addRestrictions(CriteriaBuilder criteriaBuilder, CriteriaQuery<? extends Object> criteriaQuery,
        ...
        for (Iterator<Filter> iterator = filters.iterator(); iterator.hasNext();) {
            ...
            switch (filter.getOperator()) {
                ...
                like: {
                    restrictions = criteriaBuilder.and(restrictions,
                            criteriaBuilder.like(root.<String> get(filter.getProperty()), "%" + filter.getValue() + "%"));
                    break;
                }
                case /** 包含 */
                in: {
     ...
     * @return 排序条件
     */
    private List<Order> addOrders(CriteriaBuilder criteriaBuilder, CriteriaQuery<T> criteriaQuery, List<Order> orders,
